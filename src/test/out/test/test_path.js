...
async function ollamaRequest(path, method = 'GET', body = null) {
...
const checkProcess = (0, child_process_1.spawnSync)(POWERSHELL, ['-Command', 'Get-Process ollama -ErrorAction SilentlyContinue'], { encoding: 'utf8' });
if (checkProcess.stdout && checkProcess.stdout.trim()) {
    console.log('Ollama process found in Windows, attempting to stop it...');
    const killProcess = (0, child_process_1.spawnSync)(POWERSHELL, ['-Command', 'Stop-Process -Name ollama -Force']);
...
setTimeout(() => {
    ...
    try {
        const checkFile = (0, child_process_1.spawnSync)(POWERSHELL, [
            '-Command',
        ...
        if (exists) {
            ...
            const startCommand = (0, child_process_1.spawn)(POWERSHELL, [
                '-Command',
                `
            ...
            startCommand.stdout.on('data', (data) => {
                console.log('PowerShell output:', data.toString());
            });
            startCommand.stderr.on('data', (data) => {
            ...
            setTimeout(async () => {
                try {
                    ...
                    const checkLogs = (0, child_process_1.spawnSync)(POWERSHELL, [
                        '-Command',
                        `
                    ...
                    if (checkLogs.stdout)
                        console.log(checkLogs.stdout);
                    if (checkLogs.stderr)
                        console.error(checkLogs.stderr);
...
//# sourceMappingURL=test_path.js.map