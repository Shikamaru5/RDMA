...
async function activate(context) {
    ...
    const ollamaService = new ollamaService_1.OllamaService(context);
    ...
    const modelCoordinator = new modelCoordinator_1.ModelCoordinator(ollamaService, context);
    ollamaService.setModelCoordinator(modelCoordinator);
    let currentPanel = undefined;
    const models = [
        { name: 'qwen2.5-coder', size: '7b', capabilities: ['code', 'instruction'] },
        { name: 'hermes3', size: '8b', capabilities: ['instruction', 'conversation'] },
        { name: 'llama3.2-vision', size: '11b', capabilities: ['vision', 'instruction'] }
    ];
    ...
    context.subscriptions.push(vscode.commands.registerCommand('ollama.selectModel', async () => {
        ...
        try {
            if (currentPanel) {
                ...
                currentPanel = vscode.window.createWebviewPanel('ollamaChat', 'Ollama Chat', vscode.ViewColumn.Beside, {
                    enableScripts: true,
...
class ChatViewProvider {
    constructor(_extensionUri, _ollamaService, initialModel = 'hermes3:8b') {
...
async function handleImageUpload(imageData, panel, service) {
    try {
...
async function handleCodeChanges(files, prompt, panel, service) {
    try {
...
async function handleCommandExecution(command, panel) {
    const response = await vscode.window.showWarningMessage(`Do you want to execute: ${command}?`, 'Yes', 'No');
    if (response === 'Yes') {
...
async function showChangesPreview(changes) {
    for (const change of changes) {
        ...
        for (const lineChange of change.changes) {
            const range = new vscode.Range(new vscode.Position(lineChange.lineNumber - 1, 0), new vscode.Position(lineChange.lineNumber, 0));
            edit.replace(uri, range, lineChange.newLine + '\n');
        }
        await vscode.workspace.applyEdit(edit);
    }
}
function getChatWebviewContent(context) {
    return `
            <!DOCTYPE html>
            <html lang="en">
            <head>
